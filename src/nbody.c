...
void init_particles(Universe *universe, int num_particles) {
    ...
    universe->particles = malloc(sizeof(Particle) * num_particles);

    float centerX = 320;  // Center of the distribution
    float centerY = 240;
    float maxRadius = 300;  // Maximum radius of particle distribution
...
void update_particles(Universe *universe, double dt) {
    float G = 1;  // Adjusted gravitational constant for visible effects
    ...
    // Compute gravitational force and check for collisions
    for (int i = 0; i < universe->num_particles; i++) {
